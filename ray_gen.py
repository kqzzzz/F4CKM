...
class RayGenerator:
    ...
    def get_rays(self):
        ...
        if self.cfg.sampling.known_aoa:
            ...
            # compensate rays when num_rays < k
            if len(rays_d) < 10 and self.cfg.sampling.num_extra_rays > 0:
                ...
                # rays_d_extra = fibonacci_sphere_sampling(self.cfg.sampling.num_extra_rays)
                # rays_d_extra = torch.tensor(rays_d_extra, dtype=torch.float32).to(self.device)
                rays_d = torch.cat([rays_d, rays_d_extra], dim = 0)
            rays_o = rays_o.expand(rays_d.shape)

        else:
            rays_o = self.center.to(self.device)
            if self.cfg.sampling.fibonacci:
                rays_d = fibonacci_sphere_sampling(self.cfg.sampling.n_rays)
                rays_d = torch.tensor(rays_d, device=self.device, dtype=torch.float32)
            else:
